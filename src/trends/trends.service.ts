...
export class TrendsService {
  ...
  async getTrends(payload: any) {
    ...
    console.log('📡 Query sent. Reading cursor...');
    ...
    for await (const doc of cursor) {
      ...
      if (!recordMap.has(bucketKey)) {
        ...
        for (const param of selectedParams) {
          ...
          console.log(`⚙ Calculating: ${param}`, doc);
    ...
    console.log('📊 Total Buckets Computed:', recordMap.size);
    ...
    if (mode === 'range') {
      ...
      for (let ts = startTs; ts <= endTs; ts += intervalMs) {
        ...
        merged.push(
          recordMap.get(bk) ?? {
            timestamp: bk,
            ...Object.fromEntries(selectedParams.map((p) => [p, 0])),
          },
        );
      }
    } else {
      for (let ts = startTs; ts <= endTs; ts += intervalMs) {
        ...
        merged.push(
          recordMap.get(bk) ?? {
            timestamp: bk,
            ...Object.fromEntries(selectedParams.map((p) => [p, null])),
          },
        );
      }
    }

    console.log('📈 Final Merged Length:', merged.length);

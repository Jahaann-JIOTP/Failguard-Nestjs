...
export class AianomlyService {
  ...
  private async loadCenteredWindow(
    ...
    const nextQuery: any = { timestamp: { $gt: anomalyTs } };
    ...
    return {
      previous: format(prevDocs.reverse()), // ascending order
      next: format(nextDocs), // ascending order
    };
  }
}
